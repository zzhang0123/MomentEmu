...
def test_constant_column_raises_naming_it():
    ...
    with pytest.raises(ValueError, match=r"column.*1|\[1\]"):
        PolyEmu(X, Y, cross_validation=False, max_degree_forward=2)
...
def test_object_dtype_raises_typeerror():
    ...
    with pytest.raises(TypeError, match="must be numeric"):
        PolyEmu(Xo, Y, cross_validation=False, max_degree_forward=2)
    with pytest.raises(TypeError, match="must be numeric"):
        PolyEmu([["a", "b", "c"]] * 20, Y, cross_validation=False, max_degree_forward=2)


def test_1d_y_raises_with_reshape_hint():
    X, Y = _good()
    with pytest.raises(ValueError, match="reshape to"):
        PolyEmu(X, Y.ravel(), cross_validation=False, max_degree_forward=2)
...
def test_log_Y_nonpositive_raises():
    ...
    with pytest.raises(ValueError, match="log_Y=True requires Y > 0"):
        PolyEmu(X, Y, log_Y=True, cross_validation=False, max_degree_forward=2)


def test_float32_warns_and_int_fits_as_float64():
    X, Y = _good()
    with pytest.warns(g.PrecisionWarning, match="promoting to float64"):
        emu32 = PolyEmu(X.astype(np.float32), Y, cross_validation=False, max_degree_forward=2)
    ...
    emu = PolyEmu(Xi, Yi, cross_validation=False, max_degree_forward=2)
...
def test_lone_test_array_raises():
    ...
    with pytest.raises(ValueError, match="X_test and Y_test"):
        PolyEmu(X, Y, X_test=X[:10], cross_validation=False, max_degree_forward=2)


def test_predict_rejects_non_finite_and_bad_shape():
    X, Y = _good()
    emu = PolyEmu(X, Y, cross_validation=False, max_degree_forward=2)
